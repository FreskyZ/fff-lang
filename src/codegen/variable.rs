...
impl Var {
    ...
    pub fn new(name: String, ty: TypeID, is_const: bool) -> Var {
        Var{ name: name, ty: ty, is_const: is_const, is_shadowed: false }
    }
}
                ...
                &VarOrScope::Some(ref exist_item) => {
                    if exist_item.name == item.name {
                        messages.push(CodegenMessage::VariableDefined{
                            name: item.name,
                            predef: StringPosition::new(),
                            curdef: StringPosition::new(),
                        });
...
fn gen_var_shadow() {
    ...
    macro_rules! new_var {
        ($name: expr, $id: expr, $is_const: expr) => (Var::new($name.to_owned(), TypeID::Some($id), $is_const))
    }
    ...
    assert_eq!(vars.try_push(new_var!("1", 1, false), messages), VarID::Some(0));
    ...
    let messages = &mut MessageEmitter::new();
    assert_eq!(vars.try_push(new_var!("3", 5, true), messages), VarID::Some(1));
    ...
    let messages = &mut MessageEmitter::new();
    assert_eq!(vars.try_push(new_var!("1", 5, false), messages), VarID::Invalid);
    assert_eq!(vars.items.len(), 2);
    assert_eq!(vars.find_by_name("1"), VarID::Some(0));
    assert_eq!(vars.find_by_name("3"), VarID::Some(1));
    let expect_message = &mut MessageEmitter::new();
    expect_message.push(CodegenMessage::VariableDefined{ name: "1".to_owned(), predef: StringPosition::new(), curdef: StringPosition::new() });
    assert_eq!(messages, expect_message);

    // Push barrier
    vars.push_scope();
    let messages = &mut MessageEmitter::new();
    assert_eq!(vars.try_push(new_var!("1", 13, false), messages), VarID::Some(3));
    ...
    let messages = &mut MessageEmitter::new();
    assert_eq!(vars.try_push(new_var!("42", 5, true), messages), VarID::Some(4));
    ...
    let messages = &mut MessageEmitter::new();
    assert_eq!(vars.try_push(new_var!("1", 16, false), messages), VarID::Invalid);
    assert_eq!(vars.items.len(), 5);
    assert_eq!(vars.find_by_name("1"), VarID::Some(3));
    assert_eq!(vars.find_by_name("3"), VarID::Some(1));
    assert_eq!(vars.find_by_name("42"), VarID::Some(4));

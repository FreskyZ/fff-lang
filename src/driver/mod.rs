...
pub fn main() {
    ...
    if args.is_empty() {
        println!("ffc main.f3");
    } else if let Some(main_module) = parse(scx.entry(&args[1]), &mut diagnostics) {
        ...
        while !import_requests.is_empty() {
            if let Some(source) = scx.import(import_requests[0].0, import_requests[0].1) {
                if let Some(module) = parse(source, &mut diagnostics) {

...
fn parse_array_expr() {
    ...
    //                       01234567
    case!{ parse_array_expr "[1, '2']", |_|
        make_expr!(array 0:7
            make_expr!(i32 1 1:1),
            make_expr!(char 4:6 '2')),
    }
    //                       01234567
    case!{ parse_array_expr "[1 + 1,]", |_|
        make_expr!(array 0:7
...
fn parse_binary_expr() {
    //                                     123456789012345
    case!{ parse_binary_expr "[1] * [2] / [3]", |_|
    ...
    case!{ parse_binary_expr "0 + 6 ^ 3 & 3 / 3 - 8 && 2 & 0 + 6", |_| // (((0 + 6) ^ (3 & ((3 / 3) - 8))) && (2 & (0 + 6)))
    ...
    case!{ parse_binary_expr "7 > 1 | 0 % 8 | 1 % 7 * 3 % 6 == 1 >> 8 % 3 ^ 6 << 0 ^ 2 >> 6 || 1 - 0", |_|
    ...
    case!{ parse_binary_expr "7 >> 3 == 8 / 1 && 6 == 1 <= 3 % 6 ^ 3 - 1 - 2 >> 7 || 1 >= 1", |_|
        // */%, +-, <><=>=, <<>>, &, ^, |, ==!=, &&, ||
    ...
    case!{ parse_binary_expr "8 & 0 | 7 + 7 | 7 * 0 && 1 - 2 * 3 | 0 - 7 >= 6 >> 5 % 5 || 5 % 3", |_|
    ...
    case!{ parse_binary_expr "3 <= 2 + 4 <= 5 && 3 < 3 + 2 >> 1 * 2 & 8 && 1 >= 1 < 0 || 6 < 4 * 4", |_|
    ...
    case!{ parse_binary_expr "5 >= 6 | 3 == 4 && 3", |_|
        // ((((5 >= 6) | 3) == 4) && 3)
    ...
    case!{ parse_binary_expr "6 && 7 >> 8 && 0 / 8 * 7 + 5 < 5 / 5 >> 5 - 1 >= 6 > 8 | 6 >> 5 > 2 + 1 || 0", |_|
        // */%, +-, <><=>=, <<>>, &, ^, |, ==!=, &&, ||
...
fn parse_expr() {
    ...
    case!{ parse_expr "0xfffu64", |_| make_expr!(u64 0xFFF 0:7) }
    case!{ parse_expr "'f'", |_| make_expr!(char 0:2 'f') }
    case!{ parse_expr "true", |_| make_expr!(true 0:3) }
    case!{ parse_expr "binary_expr", |x| make_expr!(x path simple 0:10 #binary_expr) }
    case!{ parse_expr "(  )", |_| make_expr!(unit 0:3) }
    ...
    //  12345678
    case!{ parse_expr "[1, 2, ]", |_|
        make_expr!(array 0:7
            make_expr!(i32 1 1:1),
            make_expr!(i32 2 4:4)),
    }
    case!{ parse_expr "[]", |_| make_expr!(array 0:1) }
    ...
    // range
    case!{ parse_expr "..", |_| make_expr!(range full 0:1) }
...
fn parse_call_expr() {
    ...
    case!{ notast parse_call_expr "()", |_| (Span::new(0, 1), Vec::new())
    }

    case!{ notast parse_call_expr "(\"hello\")", |x|
        (Span::new(0, 8), vec![
...
fn parse_array_index_expr() {
    ...
    case!{ notast parse_array_index_expr "[1, 2, ]", |_|
        (Span::new(0, 7), vec![
            make_expr!(i32 1 1:1),
            make_expr!(i32 2 4:4),
        ])
    ...
    case!{ notast parse_array_index_expr "[\"hello\"]", |x|
        (Span::new(0, 8), vec![
            make_expr!(x str #"hello" 1:7)
        ])
    }
...
fn parse_tuple_index_expr() {
    ...
    case!{ notast parse_tuple_index_expr ".0", |_| (Span::new(0, 0), (Numeric::I32(0), Span::new(1, 1))) }

    case!{ notast parse_tuple_index_expr ".0i32",
        |_| (Span::new(0, 0), (Numeric::I32(0), Span::new(1, 4))),
        // TODO: cannot have postfix
    }

    case!{ notast parse_tuple_index_expr ".0xDEADBEE",
        |_| (Span::new(0, 0), (Numeric::I32(0xDEADBEE), Span::new(1, 9))),
        // TODO: cannot have prefix
    }

    //      01234
    case!{ notast parse_tuple_index_expr ".0i8",
        |_| (Span::new(0, 0), (Numeric::I8(0), Span::new(1, 3))),
    ...
    case!{ notast parse_tuple_index_expr ".0xFFFF_FFFF_FFFF_FFFF",
        |_| (Span::new(0, 0), (Numeric::U64(0xFFFF_FFFF_FFFF_FFFF), Span::new(1, 21))),
...
fn parse_primary_expr() {
    ...
    case!{ parse_primary_expr "10363", |_| make_expr!(i32 10363 0:4) }
    ...
    case!{ parse_primary_expr "false", |_| make_expr!(false 0:4) }
    ...
    //                                      12345678
    case!{ parse_primary_expr "['f',()]", |_|
        make_expr!(array 0:7
            make_expr!(char 1:3 'f'),
            make_expr!(unit 5:6))
    }
    case!{ parse_primary_expr "[]", |_| make_expr!(array 0:1) }
...
fn parse_postfix_expr() {
    ...
    //      01234567
    case!{ parse_postfix_expr "(0, 0).0", |_|
...
fn range_expr_parse() {
    ...
    case!{ parse_range_expr "..", |_|
        make_expr!(range full 0:1)
    }

    case!{ parse_range_expr "..1 + 1", |_|
        make_expr!(range right 0:6
            make_expr!(binary 2:6 Add 4:4
                make_expr!(i32 1 2:2),
                make_expr!(i32 1 6:6)))
...
fn parse_tuple_expr() {
    //                                   01234567
    case!{ parse_tuple_expr "(1, '2')", |_|
        make_expr!(tuple 0:7
            make_expr!(i32 1 1:1),
            make_expr!(char 4:6 '2'))
    }
    //                                   0123456
    case!{ parse_tuple_expr "(1 + 1)", |_|
        make_expr!(paren 0:6
            make_expr!(binary 1:5 Add 3:3
                make_expr!(i32 1 1:1),
                make_expr!(i32 1 5:5)))
...
fn unary_expr_parse() {
    ...
    case!{ parse_unary_expr "1", |_| make_expr!(i32 1 0:0) }

    case!{ parse_unary_expr "!~!1", |_|

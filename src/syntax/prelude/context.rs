...
impl<'ecx, 'scx> ParseContext<'ecx, 'scx> {
    ...
    pub fn new(mut base: Parser<'ecx, 'scx>) -> Self {
    ...
    pub fn expect_node<N: Node>(&mut self) -> ParseResult<N::ParseOutput> {
        N::parse(self)
    }

    pub fn try_expect_node<N: Node>(&mut self) -> ParseResult<Option<N::ParseOutput>> {
        N::try_parse(self)
    }

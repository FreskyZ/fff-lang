...
enum SimpleOp {
    MemberFunctionCall(ItemID, ItemID, Vec<SimpleBase>, [Span; 2]), // fn id, ret type id, arguments, mystery pos
    MemberAccess(usize, ItemID, Span),                              // field offset, ret type
}
impl SimpleOp {
    fn final_typeid(&self) -> ItemID {
        match *self {
...
fn process_nested_full_expr(full_expr: syntax::Expr, sess: &mut GenerationSession, assigns: &mut Vec<SimpleAssignment>) -> Option<SimpleBase> {
   ...
   match (full_expr.base.as_ref().clone(), full_expr.ops.len()) {
        ...
        (_, _) => {
            ...
            Some(SimpleBase::Ident(temp_varoffset, simple_expr_typeid, Span::new()))
        }
    }
}
...
fn simplize_expr_base(expr_base: FullExpressionBase, sess: &mut GenerationSession, assigns: &mut Vec<SimpleAssignment>) 
    -> (Option<SimpleBase>, Option<String>, ItemID, Span) {  // simple_base, maybe_ident_name, current typeid, current prev pos
...
pub fn gen_expr_stmt(expr_stmt: FullExpressionStatement, sess: &mut GenerationSession, ignore_left_const: bool) {
    ...
    if expr_stmt.op.is_none() {          // expression statement
        ...
        if left_expr.ops.len() == 0 {
            sess.msgs.push(CodegenMessage::InvalidExpressionStatementSingleSimpleExpression{ pos: Span::merge(left_expr.pub_pos_all(), pos_semicolon) });
            return;
        }
        ...
        match left_expr.ops.iter().last().unwrap() {
            ...
            _ => {
                sess.msgs.push(CodegenMessage::InvalidExpressionStatementLastOpNotValid{ pos: Span::merge(left_expr.pub_pos_all(), pos_semicolon) });
                return;
            }
        }
        ...
        match sess.vars.find_by_id(left_varid) {
            Some(ref var) => if var.is_const && !ignore_left_const { // is const but not ignore const
                sess.msgs.push(CodegenMessage::AssignToConstVar{
                    name: var.name.clone(),
                    pos: Span::merge(left_expr_pos, semicolon_pos),
                });
            },
...
fn gen_expr_pure_simple_test() {
    ...
    // test_case!{ "1",
    //     Ok(SimpleBase::Lit(LitValue::from(1), ItemID::new(5), make_span!(0, 0)))
    // }
    // test_case!{ "2u32", 
    //     Ok(SimpleBase::Lit(LitValue::from(2u32), ItemID::new(6), make_span!(0, 3)))
    // }
    // test_case!{ "\"2f32\"", 
    //     Ok(SimpleBase::Lit(LitValue::from("2f32"), ItemID::new(13), make_span!(0, 5)))
    // }
    // test_case!{ "1 as u32", 
...
fn gen_expr_simple_test() {
    ...
    expect_message.push(CodegenMessage::FunctionCallOperatorNotAppliedToIdentifier{ pos: make_span!(1, 2) });
    assert_eq!(final_expr, None);
    assert_eq!(&sess.msgs, expect_message);

    // subscription 1 parameter          1234567
    let expr = make_node!("a[2, 3]");
    let mut sess = GenerationSession::new();
    let mut assigns = Vec::new();
    ...
    macro_rules! prepare_vars { 
        ($sess: expr, $($name: expr, )*) => (
            $(
                $sess.vars.try_push(Var::new($name.to_owned(), ItemID::new(1), false, Span::new()), &mut $sess.types, &mut $sess.msgs);
            )*
        )
    }
...
fn gen_expr_practice_test() {
    ...
    let right_expr = make_node!("x * x + y * y - 1");
    let mut assigns = Vec::new();
    let mut sess = GenerationSession::new();
    ...
    let right_expr = make_node!("a * a * a - x * x * y * y * y <= 0f32");
    let mut assigns = Vec::new();
    let mut sess = GenerationSession::new();
    let final_expr = simplize_expr(right_expr, &mut sess, &mut assigns);
    perrorln!("messages: \n{:?}", sess.msgs);

...
impl ISemanticAnalyze for Module {
    ...
    fn from_syntax(node: syntax::Module, this_scope: SharedDefScope) -> Module {
        Module{
            module_id: node.source.get_file_id(),
            items: node.items.into_iter().map(|item| Item::from_syntax(item, this_scope.clone())).collect(),
...
impl Module {
    ...
    pub fn buildup_imports(&mut self, import_maps: &Vec<syntax::ImportMap>, modules: &mut Vec<syntax::Module>, symbols: &SymbolCollection) {

...
// fn process_nested_full_expr(full_expr: BinaryExpr, sess: &mut GenerationSession, assigns: &mut Vec<SimpleAssignment>) -> Option<SimpleBase> {

//     match Some(&full_expr).and_then(BinaryExpr::get_unary).and_then(UnaryExpr::get_postfix).and_then(PostfixExpr::get_primary) {
//         Some(primary_expr) => {
//             if primary_expr.is_lit() {
//                 let lit_value = primary_expr.get_lit_value().unwrap();
//                 let lit_typeid = TypeCollection::get_id_by_lit(lit_value);
//                 return Some(SimpleBase::Lit(lit_value.clone(), lit_typeid, primary_expr.get_all_strpos()));
//             } else if primary_expr.is_ident() {
//                 let ident_name = primary_expr.get_ident_name().unwrap();
//                 let ident_varid = sess.vars.find_by_name(ident_name);
//                 let ident_typeid = sess.vars.get_type(ident_varid);
//                 let ident_offset = sess.vars.get_offset(ident_varid);
//                 return Some(SimpleBase::Ident(ident_offset, ident_typeid, primary_expr.get_all_strpos()));
//             }
//         }
//         None => (),
//     }
...
//     return Some(SimpleBase::Ident(temp_varoffset, simple_expr_typeid, StringPosition::new()));
// }
...
// fn simplize_expr_base(primary_expr: PrimaryExpr, sess: &mut GenerationSession, assigns: &mut Vec<SimpleAssignment>) 
//     -> (Option<SimpleBase>, Option<String>, ItemID, StringPosition) {  // simple_base, maybe_ident_name, current typeid, current prev pos

//     let current_prev_pos = primary_expr.get_all_strpos();

//     if primary_expr.is_lit() {
//         let lit_value = primary_expr.get_lit_value().unwrap();
//         let lit_typeid = TypeCollection::get_id_by_lit(lit_value);
//         return (Some(SimpleBase::Lit(lit_value.clone(), lit_typeid, primary_expr.get_all_strpos())), None, lit_typeid, current_prev_pos);
//     } else if primary_expr.is_ident() {
//         let ident_name = primary_expr.get_ident_name().unwrap();
//         let ident_varid = sess.vars.find_by_name(&ident_name);
//         let ident_offset = sess.vars.get_offset(ident_varid);
//         let ident_typeid = sess.vars.get_type(ident_varid);
//         return (Some(SimpleBase::Ident(ident_offset, ident_typeid, primary_expr.get_all_strpos())), Some(ident_name.clone()), ident_typeid, current_prev_pos);
//     } else if primary_expr.is_paren() { // directly move to temp assignment
//         let _inner_expr = primary_expr.get_paren_inner().unwrap();
//         // TODO: expr do not have copy
//         // match process_nested_full_expr(inner_expr, sess, assigns) { 
...
//         //     }
//         // }
//     } else if primary_expr.is_array_dup() { // convert expr1 and expr2 to simple base and act like a global call to ?new_array
//         let _expr1 = primary_expr.get_array_dup_element().unwrap();
//         let _expr2 = primary_expr.get_array_dup_count().unwrap();
//         let _expr2_strpos = _expr2.get_all_strpos();
//         // TODO: expr do not have copy
...
//         //     }
//         // }
//     } else if primary_expr.is_tuple() {
//         let exprs = primary_expr.get_tuple_inners();
...
//             return (Some(SimpleBase::FunctionCall(ctor_fnid, tuple_typeid, simple_bases, [primary_expr.get_all_strpos(), primary_expr.get_all_strpos()])), None, tuple_typeid, current_prev_pos);
//         } else {
//             return (None, None, ItemID::new_invalid(), current_prev_pos);
//         }
//     }
        ...
        // PrimaryExpr::ArrayDef(exprs, _pos) => {
...
// fn simplize_expr(expr: BinaryExpr, sess: &mut GenerationSession, assigns: &mut Vec<SimpleAssignment>) -> Option<SimpleExpr> {
...
//     // for op in expr.ops {
    //     match op {
    //         BinaryExprOperator::FunctionCall(exprs, pos) => {
    ...
    //             }
    //         }
    //         BinaryExprOperator::MemberAccess(name, pos) => {
...
pub fn gen_expr(expr: BinaryExpr, sess: &mut GenerationSession) -> (Operand, ItemID) {
   
   let _ = expr;
   let _ = sess;
   (Operand::Unknown, ItemID::new_invalid())
...
pub fn gen_expr_stmt(expr_stmt: BinaryExprStatement, sess: &mut GenerationSession, ignore_left_const: bool) {
    
    let _ = expr_stmt;
    let _ = sess;
    let _ = ignore_left_const;
    ...
    //     //     &BinaryExprOperator::FunctionCall(_, _) 
    //     //     | &BinaryExprOperator::MemberFunctionCall(_, _, _) => (),
    //     //     // &BinaryExprOperator::Unary(SeperatorKind::Increase, _)
    //     //     // | &BinaryExprOperator::Unary(SeperatorKind::Decrease, _) => {
    //     //     //     let mut left_ident_name = None;
    //     //     //     if left_expr.ops.len() == 1 { // only ++a is acceptable, ++a[10] is not
    //     //     //         match left_expr.base.as_ref() {
    //     //     //             &PrimaryExpr::Ident(ref name, _) => left_ident_name = Some(name.clone()),
    //     //     //             _ => (),
    //     //     //         }
    //     //     //     }
...
fn gen_expr_pure_simple_test() {
    ...
    //         assert_eq!(
    //             check_pure_simple_expr(BinaryExpr::from_str($prog, 0), &mut GenerationSession::new()),
    //             $result
    //         );
    //     )
    // }
    ...
    // }
    // test_case!{ "1 as u32", 
    //     Err(BinaryExpr::from_str("1 as u32", 0))
    // }
}
...
fn gen_expr_simple_test() {
    ...
    // let expr = BinaryExpr::with_test_str("1()");
    ...
    // let expr = BinaryExpr::with_test_str("a[2, 3]");
    // let mut sess = GenerationSession::new();
    // let mut assigns = Vec::new();
    ...
    // let expr = BinaryExpr::with_test_str(
...
fn gen_expr_practice_test() {
    ...
    // let right_expr = BinaryExpr::with_test_str("x * x + y * y - 1");
    // let mut assigns = Vec::new();
    // let mut sess = GenerationSession::new();
    ...
    // let right_expr = BinaryExpr::with_test_str("a * a * a - x * x * y * y * y <= 0f32");
    // let mut assigns = Vec::new();
    // let mut sess = GenerationSession::new();
    // let final_expr = simplize_expr(right_expr, &mut sess, &mut assigns);
    // perrorln!("messages: \n{:?}", sess.msgs);
...
fn gen_expr_stmt_inter() {
    ...
    loop {
        ...
        if buf != "break\r\n" {
            match BinaryExprStatement::with_test_str_ret_size(&buf).0 {
                Some(expr_stmt) => {
